...
var windowLength = process.argv[4] ? parseInt(process.argv[4]) : 16
...
function calcSpeed() {
	...
	setInterval(function() {
		...
		var lostRate = (packetReceivedCount / (lastPacketSeq + 1) * 100).toFixed(2)
		//console.log('speed %s B/s, lost %s%%', speed, lostRate)
		console.log('speed %s B/s, avrage speed %s B/s, sended %s packets, received %s packets, lost rate %s', speed, speedAverage, lastPacketSeq + 1, packetReceivedCount, lostRate)
		byteSended = 0
	}, t * 1000)
}
...
function sendPacket() {
	...
	setTimeout(sendPacket, lostTimeout)
